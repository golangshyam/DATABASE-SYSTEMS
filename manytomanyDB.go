...
type Clientoou struct {
	gorm.Model
	Company   string
	Trainings []Trainingeee `json:"trainings" gorm:"foreignkey:Refer"`
}

type Trainingeee struct {
	gorm.Model
	Refer        uint
	Technology   string
	Participants string
	pp           []Clientoou `gorm:"many2many:clientoou_trainingeee"`
}
...
func Constuctor() Technology {
	...
	db, err := gorm.Open("mysql", "root:shyamvarma@tcp(127.0.0.1:3306)/manytomany?charset=utf8&parseTime=True")
	...
	db.AutoMigrate(&Clientoou{}, &Trainingeee{})
...
func APIvalidation(inputs *TrainingRequest, req *Trainings) bool {
	...
	*/
	...
	req.db.Create(&Clientoou{Company: "netflix", Trainings: []Trainingeee{{Technology: "golang", Participants: "freshers"}}})
